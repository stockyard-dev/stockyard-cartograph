...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/sitemaps", s.list)
	s.mux.HandleFunc("POST /api/sitemaps", s.create)
	s.mux.HandleFunc("GET /api/sitemaps/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/sitemaps/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-cartograph", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"sitemaps": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Sitemap{} }
	wj(w, map[string]any{"sitemaps": items, "count": len(items)})
}
